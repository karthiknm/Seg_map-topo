...
# Ensure GPU cache is cleared before training
...
parser.add_argument('--hemi', required=True, help='hemisphere to train with (lh or rh)')
...
# configure dataset sampler to load the segmentation maps
data_iterator = iter(topofit.io.get_data_loader(args.hemi, training_subjs, model.low_res_training))
   ...
   if epoch % validation_epochs == 0 and epoch != initial_epoch:
       ...
       with torch.no_grad():
           for subj in validation_subjs:
               data = topofit.io.load_subject_data(subj, args.hemi, ground_truth=True)
